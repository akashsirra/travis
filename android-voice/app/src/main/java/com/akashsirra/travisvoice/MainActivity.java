...
public class MainActivity extends android.app.Activity {
    ...
    @Override public void onCreate(Bundle state) {
        ...
        TextView title=new TextView(this); title.setText("TRAVIS VOICE v1.4"); title.setTextSize(28); title.setGravity(Gravity.CENTER); root.addView(title,new LinearLayout.LayoutParams(-1,-2));
        status=new TextView(this); status.setText("Native speech recognition • Google TTS"); status.setTextSize(16); status.setGravity(Gravity.CENTER); root.addView(status,new LinearLayout.LayoutParams(-1,-2));
    ...
    private void testVoice(){
        ...
        holder[0]=new android.speech.tts.TextToSpeech(this,result->{
            android.speech.tts.TextToSpeech t=holder[0];
            if(result==android.speech.tts.TextToSpeech.SUCCESS){
                t.setLanguage(java.util.Locale.US); t.setPitch(0.48f); t.setSpeechRate(0.78f);
                android.speech.tts.Voice selected=null; java.util.Set<android.speech.tts.Voice> voices=t.getVoices();
                String[] preferred={"en-us-x-sfg#male_1-local","en-us-x-sfg#male_2-local","en-us-x-tpc#male_1-local","en-us-x-tpc#male_2-local"};
                if(voices!=null){for(String wanted:preferred){for(android.speech.tts.Voice v:voices){if(v!=null&&wanted.equalsIgnoreCase(v.getName())){selected=v;break;}}if(selected!=null)break;}}
                if(selected!=null){t.setVoice(selected);t.setPitch(0.48f);t.setSpeechRate(0.78f);status.setText("Travis voice: "+selected.getName());}
                else status.setText("No explicit Google male voice installed");
                t.speak("Yeah. Travis is online.",android.speech.tts.TextToSpeech.QUEUE_FLUSH,null,"test");
            }else status.setText("TTS initialization failed");
