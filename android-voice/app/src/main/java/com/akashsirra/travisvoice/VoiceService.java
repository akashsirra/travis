...
public class VoiceService extends Service implements RecognitionListener {
    ...
    private boolean commandMode=false,wakeDispatched=false,stopping=false,listening=false,startScheduled=false;
        ...
        try{
            ...
            if(voices!=null){
                ...
                if(selected==null)for(Voice v:voices){
                    ...
                    Locale l=v.getLocale(); String n=v.getName()==null?"":v.getName().toLowerCase(Locale.ROOT); String f=v.getFeatures()==null?"":v.getFeatures().toString().toLowerCase(Locale.ROOT);
            ...
            else Log.w(TAG,"NO EXPLICIT GOOGLE MALE VOICE FOUND; using default US voice");
        }catch(Exception e){Log.e(TAG,"voice setup",e);}
    }

    // One recognition session at a time. Do not cancel/start from partial results.
    ...
    private void handleFinal(String raw){
        ...
        if(!commandMode){
            ...
            if(p<0){scheduleListen(300);return;}
            wakeDispatched=true;
    ...
    @Override public void onError(int e){listening=false;Log.w(TAG,"recognition error: "+e);scheduleListen(700);}
